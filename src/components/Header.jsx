...
const Header = () => {
  ...
  useEffect(() => {
    ...
    }, 15000); // Change image and text every 15 seconds
  ...
  return (
    ...
    >
      ...
      <div
        className={`absolute inset-0 bg-cover bg-center transition-all duration-10000 ease-in-out background-transition ${
      ...
      <motion.div
        key={currentImageIndex} // Force re-render and restart animation when the image changes
        initial={{ opacity: 0, y: 100 }} // Initial state of the animation
        transition={{ duration: 1.5 }} // Animation duration
        whileInView={{ opacity: 1, y: 0 }} // Animate when in view
        viewport={{ once: false }} // Allow animation to repeat for each new image
        className="container text-center mx-auto px-6 py-4 md:px-20 lg:px-32 text-white relative z-10"
      >
        <h2
        ...
        <div className="space-x-6 mt-16">
          <a
            href="#Projects"
            className="border border-white px-8 py-3 rounded hover:bg-black cursor-pointer"
          >
            Projects
          </a>
          <a
            href="#Contact"
            className="bg-blue-500 px-8 py-3 rounded hover:bg-black cursor-pointer"
          >
            Contact Us
          </a>
        </div>
